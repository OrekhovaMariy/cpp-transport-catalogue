...
namespace transport_catalogue::input_reader
{
    void ProcessInput(TransportCatalogue& tc, std::istream& is)
    {
        ...
        for (size_t i = 0; i < request_num; ++i)
        {
            ...
            {
                query.inputtype_ = InputQueryType::AddStop;
                ...
                if (tmp.second.size() != 0)
                {
                    ...
                    query.query_ = std::string(tmp.first);
                    queries.push_back(std::move(query));
                    query = {};
                    query.inputtype_ = InputQueryType::AddStopsDistance;
                    query.query_ = std::string(tmp_stop_name.first) + ":"s + std::string(tmp.second);
                    queries.push_back(std::move(query));
                }
                else
                {
            ...
            {
                query.inputtype_ = InputQueryType::AddRoute;
                query.query_ = std::string(tmp.second);
                queries.push_back(std::move(query));
            }
            else
            {
    ...
    void ProcessInputQueries(TransportCatalogue& tc, std::vector<InputQuery>& queries)
    {
        ...
        {
            if (element.inputtype_ == InputQueryType::AddStop)
            {
                tc.AddStop(ProcessQueryAddStop(element.query_));
            }
            else
            {
        ...
        for (auto& element : queries)
        {
            if (element.inputtype_ == InputQueryType::AddStopsDistance)
            {
                ProcessQueryAddStopsDistance(tc, element.query_);
            }
            else
            {
                delayed.push_back(std::move(element));
        ...
        {
            if (element.inputtype_ == InputQueryType::AddRoute)
            {
                tc.AddRoute(std::move(ProcessQueryAddRoute(tc, element.query_)));
            }
        }
    }
